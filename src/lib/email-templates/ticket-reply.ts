...
export function getTicketReplyEmail(data: TicketReplyEmailData): {
  ...
  const { customerName, ticketNumber, subject, agentName, replyContent, dashboardUrl } = data;
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
          ...
          <tr>
            <td style="background-color: #2563eb; padding: 30px 40px; text-align: center;">
              <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 600;">LLCPad Support</h1>
            </td>
          </tr>
            ...
            <td style="padding: 40px;">
              ...
              <div style="background-color: #f0f9ff; border-left: 4px solid #2563eb; padding: 20px; margin: 25px 0; border-radius: 0 8px 8px 0;">
                <div style="display: flex; align-items: center; margin-bottom: 15px;">
                  <div style="width: 36px; height: 36px; background-color: #2563eb; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-right: 12px;">
                    <span style="color: #ffffff; font-weight: 600; font-size: 14px;">${agentName.split(' ').map(n => n[0]).join('').toUpperCase()}</span>
                  </div>
                  <div>
                    <p style="color: #18181b; margin: 0; font-size: 14px; font-weight: 600;">${agentName}</p>
                    <p style="color: #71717a; margin: 0; font-size: 12px;">LLCPad Support Team</p>
                  </div>
                </div>
